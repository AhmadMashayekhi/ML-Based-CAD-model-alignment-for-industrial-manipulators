...
# print(BASE_DIR)
import torch                      # you call torch.cuda.empty_cache() and torch.load(...)
from pathlib import Path          # to resolve project-relative paths robustly
...
def main(source_tensor, template_tensor):
    ...
    # Resolve pretrained model path under <repo>/Input_Output/checkpoints/exp_pnlk/models/best_model.t7
    # This file lives in <repo>/src/methods/, so project root is parents[2]
    PROJECT_ROOT = Path(__file__).resolve().parents[2]
    default_pretrained = PROJECT_ROOT / "Input_Output" / "checkpoints" / "exp_pnlk" / "models" / "best_model.t7"

    # Allow overrides via module attribute or environment variable if desired
    override = globals().get("MODEL_PATH", None) or os.environ.get("PNLK_PRETRAINED", None)
    args.pretrained = str(override if override else default_pretrained)
    ...
    if args.pretrained:
        if not os.path.isfile(args.pretrained):
            raise FileNotFoundError(
                f"PointNetLK pretrained model not found at:\n  {args.pretrained}\n"
