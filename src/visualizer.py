...
def apply_transformation(template_pointcloud, source_pointcloud, transformation_matrix):
    ...
    # Small local frame (for reference)
    local_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.02)
    
    # Larger global frame at origin (XYZ axes)
    global_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.2)
    
    # Color the clouds
    source_pointcloud.paint_uniform_color([1, 0, 0])              # Red = source
    source_pointcloud_transformed.paint_uniform_color([1, 0, 0])  # Red = transformed
    template_pointcloud.paint_uniform_color([0, 0, 1])            # Blue = template
    tmpl1 = copy.deepcopy(template_pointcloud)
    tmpl1.paint_uniform_color([0, 1, 0])                          # Green = template copy

    # Show before and after with global XYZ axes visible
    o3d.visualization.draw_geometries(
        [template_pointcloud, source_pointcloud, global_frame],
        window_name="Original Point Clouds"
    )

    o3d.visualization.draw_geometries(
        [tmpl1, source_pointcloud_transformed, global_frame],
...
#     source_pointcloud.paint_uniform_color([1,0,0])                        # Red = source = scan/PLY / 12 mesh
#     source_pointcloud_transformed.paint_uniform_color([1,0,0])           # Green = transformed scan/PLY / 12 mesh    
#     template_pointcloud.paint_uniform_color([0,0,1])                      # Blue = CAD/template / 3 meshes
#     tmpl1 = copy.deepcopy(template_pointcloud)
#     tmpl1.paint_uniform_color([0, 1, 0])   
    
    
#     o3d.visualization.draw_geometries([template_pointcloud,source_pointcloud, coordinate_frame], 
#                                       window_name = "Original Point Clouds")
    
#     o3d.visualization.draw_geometries([tmpl1,source_pointcloud_transformed,coordinate_frame], 
#                                       window_name = "Transformation")
