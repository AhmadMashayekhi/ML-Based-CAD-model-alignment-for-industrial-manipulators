...
MODEL_PATH = PROJECT_ROOT / "Input_Output" / "checkpoints" / "exp_pnlk" / "models" / "best_model.t7"
...
def run_method(template, source, pcr_method, MSEThresh, trimFraction, voxel_size, zero_mean):
    ...
    if pcr_method == "ICP":
        ...
        transformation, registration_time = GO_ICP.main(
            source_tensor[:, :, 0:3], template_tensor[:, :, 0:3]
        )
        ...
        # Prefer model inside project: <repo>/Input_Output/checkpoints/exp_pnlk/models/best_model.t7
        PROJECT_ROOT = Path(__file__).resolve().parents[1]
        MODEL_PATH = PROJECT_ROOT / "Input_Output" / "checkpoints" / "exp_pnlk" / "models" / "best_model.t7"

        # Try explicit kwarg first; fall back to module attribute if older signature
        try:
            transformation, registration_time = PointNetLK.main(
                source_tensor[:, :, 0:3],
                template_tensor[:, :, 0:3],
                model_path=str(MODEL_PATH)
            )
        except TypeError:
            setattr(PointNetLK, "MODEL_PATH", str(MODEL_PATH))
            transformation, registration_time = PointNetLK.main(
                source_tensor[:, :, 0:3],
                template_tensor[:, :, 0:3]
            )
