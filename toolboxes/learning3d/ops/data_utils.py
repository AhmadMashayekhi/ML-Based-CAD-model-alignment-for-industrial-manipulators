...
def mean_shift(template, source, p0_zero_mean, p1_zero_mean):
    ...
    if p0_zero_mean:
        ...
        template_mean = torch.cat([template_mean, p0_m.unsqueeze(-1)], dim=2) #add mean as translation
        #print(template_mean)
        one_ = torch.tensor([[[0.0, 0.0, 0.0, 1.0]]]).repeat(template_mean.shape[0], 1, 1).to(template_mean)    # (Bx1x4)
        template_mean = torch.cat([template_mean, one_], dim=1) #add bottom row in Homog. transfo
    ...
    if p1_zero_mean:
        ...
        source_mean = torch.cat([source_mean, -p1_m.unsqueeze(-1)], dim=2) #changed -p0_m to -p1_m
        ...
        #print(p1_m) real mean -> important for translation
    # else:
        # q1 = source
    #print(template_mean)
