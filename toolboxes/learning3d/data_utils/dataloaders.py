...
def load_data(train, use_normals):
    ...
    h5s = glob.glob(os.path.join(modelnet_dir, f'ply_data_{partition}*.h5'))
    if not h5s:
        raise FileNotFoundError(
            f"No ModelNet40 files found in {modelnet_dir}. "
            f"Either pass download=True to ModelNet40Data, or don’t use ModelNet in this run."
        )
    ...
    for h5_name in h5s:
        with h5py.File(h5_name, 'r') as f:
            ...
            if use_normals:
                data = np.concatenate([data, f['normal'][:]], axis=-1).astype('float32')
            label = f['label'][:].astype('int64')
        all_data.append(data)
        all_label.append(label)

    return np.concatenate(all_data, 0), np.concatenate(all_label, 0)
...
class ClassificationData(Dataset):
    def __init__(self, data_class=None):
        ...
        if data_class is None:
            raise ValueError("ClassificationData requires a data_class; "
                             "don’t default to ModelNet40Data.")
...
class RegistrationData(Dataset):
    def __init__(self, algorithm, data_class=None,
                 partial_source=False, partial_template=False, noise=False,
        ...
        if data_class is None:
            raise ValueError("RegistrationData requires a data_class; "
                             "don’t default to ModelNet40Data.")
        self.set_class(data_class)
