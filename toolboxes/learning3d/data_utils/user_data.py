...
class ClassificationData:
    def __init__(self, data_dict):
        ...
        self.pcs = self.h5_reader('pcs')
        self.labels = self.h5_reader('labels')
        self.check_data()
    
    def h5_reader(self,key):
        
        with h5py.File(self.data_dict,"r") as f:
            if(key in f.keys()):
                dataset = f[key][()]
            else:
...
class RegistrationData:
    def __init__(self, data_dict):
        self.data_dict = data_dict
        self.template = self.h5_reader("template")
        self.source = self.h5_reader("source")
        self.transformation = self.h5_reader("transformation")
        self.check_data()
