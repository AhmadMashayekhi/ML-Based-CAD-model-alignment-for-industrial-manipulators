...
class PointNet(torch.nn.Module):
    def __init__(self, emb_dims=1024, input_shape="bnc", use_bn=False, global_feat=True):
        # use_bn               Use Batch Normalization (faster training of NN)
    ...
    def create_structure(self):
        self.conv1 = torch.nn.Conv1d(3, 64, 1)  #Applies a 1D convolution over an input signal 
        self.conv2 = torch.nn.Conv1d(64, 64, 1) #composed of several input planes.
        self.conv3 = torch.nn.Conv1d(64, 64, 1)
        self.conv4 = torch.nn.Conv1d(64, 128, 1)
        self.conv5 = torch.nn.Conv1d(128, self.emb_dims, 1)
        self.relu = torch.nn.ReLU()
        
        #Wikipedia: "Batch normalization (also known as batch norm) is a method used to make 
        #training of artificial neural networks faster and more stable through normalization 
        #of the layers' inputs by re-centering and re-scaling."
        
        if self.use_bn:
            self.bn1 = torch.nn.BatchNorm1d(64) #bn1...bn5 apply batchnorm on each layer
            self.bn2 = torch.nn.BatchNorm1d(64) #size = number of features
            self.bn3 = torch.nn.BatchNorm1d(64) #affine = True => With Learnable Parameters
